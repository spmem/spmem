...
class VideoDatasetWithResizingTrackingWorldModel(VideoDataset):
    def __init__(self, *args, **kwargs) -> None:
        self.tracking_column = kwargs.pop("tracking_column", None)
        super().__init__(*args, **kwargs)
        self.needgen_frames = self.max_num_frames - self.cond_frames

    def _preprocess_video(self, path: str, tracking_path: str) -> torch.Tensor:
        ...
        if self.load_tensors:
            ...
            video_num_frames_tracking = len(tracking_reader)
            tracking_frame_indices = list(range(0, video_num_frames_tracking))
            tracking_frames = tracking_reader.get_batch(tracking_frame_indices)
            tracking_frames = tracking_frames.float()
            tracking_frames = tracking_frames.permute(0, 3, 1, 2).contiguous()
            nearest_res = self._find_nearest_resolution(tracking_frames.shape[2], tracking_frames.shape[3])
            ...
            video_reader = decord.VideoReader(uri=path.as_posix())
            video_num_frames = len(video_reader)
            frame_indices = list(range(0, video_num_frames))
            ...
            ### modified ###
            video_mt_frames_raw = tracking_frames.clone()
            video_source_frames_raw = frames[:self.max_num_frames]
            video_target_frames_raw = frames[self.max_num_frames-1:] #[49, 3, 480, 720]


            video_target = torch.cat([video_source_frames_raw[-self.cond_frames:], video_target_frames_raw[1:self.needgen_frames+1]], dim=0)
            video_mt = torch.cat([video_source_frames_raw[-self.cond_frames:], video_mt_frames_raw[1:self.needgen_frames+1]], dim=0)
            
            cond_images = video_source_frames_raw[-self.cond_frames:].clone() 
            reference = video_source_frames_raw[-self.ref_frames:].clone() 


            del video_reader, tracking_reader
            del video_mt_frames_raw, video_source_frames_raw, video_target_frames_raw
            
            
            return reference, cond_images, video_target, video_mt, None
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        prompt_path = self.caption_column
        video_path = self.video_column
        tracking_path = self.tracking_column


        with open(prompt_path, "r", encoding="utf-8") as file:
            prompts = [line.strip() for line in file.readlines() if len(line.strip()) > 0]
        with open(video_path, "r", encoding="utf-8") as file:
            video_paths = [line.strip() for line in file.readlines() if len(line.strip()) > 0]
            # video_paths = [self.data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        with open(tracking_path, "r", encoding="utf-8") as file:
            tracking_paths = [line.strip() for line in file.readlines() if len(line.strip()) > 0]
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        reference, cond_images, video_target, video_mt, _ = self._preprocess_video(self.video_paths[index], self.tracking_paths[index])
        ...
        return {
            ...
            "image": cond_images,
            "video": video_target,
            "tracking_map": video_mt,
            "video_metadata": {
                "num_frames": video_target.shape[0],
                "height": video_target.shape[2],
                "width": video_target.shape[3],
            },
        }
