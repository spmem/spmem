...
#
# Inference overview:
# This script expects a single long image sequence (e.g. all frames from one video in one
# directory). It splits the sequence into overlapping chunks (chunk_size and overlap from config),
# runs DepthAnything3 on each chunk to get per-frame depth, confidence, intrinsics and
# extrinsics, then aligns consecutive chunks in 3D via Sim(3) on the overlap regions (and
# optionally loop-closure constraints). All chunks are transformed into the first chunk’s
# coordinate frame, per-chunk point clouds are written to pcd/, merged into combined_pcd.ply,
# and global camera_poses.txt / intrinsic.txt are saved. Best used for one long sequence per run;
# very short sequences (single chunk) may yield no valid poses/PLY due to overlap trimming.
...
class DA3_Streaming:
    ...
    def save_camera_poses(self):
        ...
        print("Saving all camera poses to txt / ply files...")

        all_poses = [None] * len(self.img_list)
        all_intrinsics = [None] * len(self.img_list)

        # First chunk (reference chunk)
        ...
        # Remaining chunks (aligned to the first chunk via Sim(3))
        ...
        # Filter out any frames that did not get a valid pose/intrinsic
        valid_entries = [
            (pose, intrinsic)
            for pose, intrinsic in zip(all_poses, all_intrinsics)
            if pose is not None and intrinsic is not None
        ]

        if len(valid_entries) == 0:
            print("Warning: no valid camera poses found, skip saving camera_poses files.")
            return

        # Save camera poses (txt)
        poses_path = os.path.join(self.output_dir, "camera_poses.txt")
        with open(poses_path, "w") as f:
            for pose, _intrinsic in valid_entries:
        ...
        with open(intrinsics_path, "w") as f:
            for _pose, intrinsic in valid_entries:
                fx = intrinsic[0, 0]
                fy = intrinsic[1, 1]
                cx = intrinsic[0, 2]
                cy = intrinsic[1, 2]
        ...
        # Save camera poses as PLY
        ply_path = os.path.join(self.output_dir, "camera_poses.ply")
        with open(ply_path, "w") as f:
            # Write PLY header
            f.write("ply\n")
            f.write("format ascii 1.0\n")
            f.write(f"element vertex {len(valid_entries)}\n")
