...
def cal_reasonable_voxel_size(vol_bnds, voxel_size_now, max_vol_dim=1200):
  
  vol_dim = np.ceil((vol_bnds[:,1]-vol_bnds[:,0])/ voxel_size_now).copy(order='C').astype(int)

  max_vol_dim_now = max(vol_dim[0] , vol_dim[1] , vol_dim[2])
  print("max_vol_dim_now:", max_vol_dim_now)


  scale = float(max_vol_dim_now / max_vol_dim)
  print("scale:", scale, scale * voxel_size_now)
  return voxel_size_now * scale

  # if max_vol_dim_now <= max_vol_dim:
  #   return voxel_size_now
  # else:
  #   scale = float(max_vol_dim_now / max_vol_dim)
  #   print("scale:", scale, scale* voxel_size_now)
  #   return voxel_size_now * scale
...
class TSDFVolume:
  ...
  """
  def __init__(self, vol_bnds, voxel_size, max_vol_dim=1200, use_gpu=True):
    """Constructor.
    ...
    self._voxel_size = float(cal_reasonable_voxel_size(self._vol_bnds, float(voxel_size), max_vol_dim=max_vol_dim))
